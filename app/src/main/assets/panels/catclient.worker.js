...
const EDGE_LOCATIONS = {
  ...
  ATH: { city: 'Athens', country: 'Greece', flag: '🇬🇷' },
  AUH: { city: 'Abu Dhabi', country: 'United Arab Emirates', flag: '🇦🇪' },
  BAH: { city: 'Manama', country: 'Bahrain', flag: '🇧🇭' },
  BCN: { city: 'Barcelona', country: 'Spain', flag: '🇪🇸' },
  BER: { city: 'Berlin', country: 'Germany', flag: '🇩🇪' },
  BOM: { city: 'Mumbai', country: 'India', flag: '🇮🇳' },
  BRU: { city: 'Brussels', country: 'Belgium', flag: '🇧🇪' },
  ...
  DUS: { city: 'Dusseldorf', country: 'Germany', flag: '🇩🇪' },
  ...
  HEL: { city: 'Helsinki', country: 'Finland', flag: '🇫🇮' },
  HKG: { city: 'Hong Kong', country: 'Hong Kong', flag: '🇭🇰' },
  IAD: { city: 'Washington', country: 'United States', flag: '🇺🇸' },
  ...
  LON: { city: 'London', country: 'United Kingdom', flag: '🇬🇧' },
  MAD: { city: 'Madrid', country: 'Spain', flag: '🇪🇸' },
  MEL: { city: 'Melbourne', country: 'Australia', flag: '🇦🇺' },
  MEX: { city: 'Mexico City', country: 'Mexico', flag: '🇲🇽' },
  ...
  OSL: { city: 'Oslo', country: 'Norway', flag: '🇳🇴' },
  PRG: { city: 'Prague', country: 'Czechia', flag: '🇨🇿' },
  ...
  TPE: { city: 'Taipei', country: 'Taiwan', flag: '🇹🇼' },
  TLV: { city: 'Tel Aviv', country: 'Israel', flag: '🇮🇱' },
...
function locationFromColo(colo) {
  ...
  return EDGE_LOCATIONS[key] || (key ? { city: key, country: 'Edge', flag: '🌐' } : { city: 'Auto', country: 'Global', flag: '🌐' });
}
...
function configOptions(url, host, env, settings) {
  ...
  const country = String(q.get('country') || cfg.country || env.COUNTRY || '').trim().toUpperCase();
  return { addresses: addresses, ports: ports, sni: sni, protocols: protocols.length ? protocols : ['vless'], includeHost: includeHost, fragment: fragment, fingerprint: fingerprint, includeIpv6: includeV6, locations: locations, country: country };
}
...
function configName(kind, addr, port, index, host, options) {
  ...
  return '🐱 Cat · ' + location.flag + ' ' + location.country + ' · ' + label + ' · ' + port + ' · #' + String(index).padStart(2, '0');
}
...
async function probeIp(ip, timeoutMs = 4000, host) {
  ...
  try {
    ...
    return { ip: ip, ok: ok, ms: ms, status: answer.status, colo: colo, location: location, cf: cfServer };
  } catch (e) {
    clearTimeout(timer);
...
function panelClientJs() {
  return [
    ...
    '  var loc=locationForAddr(h);var name="🐱 Cat · "+loc.country+" "+loc.flag+" · "+(k==="vless"?"VLESS":"Trojan")+" · "+p;',
    ...
    '$("#cfgFromScan").addEventListener("click",function(){var picked=scanResults.filter(function(r){return r.ms!==null}).sort(function(a,b){return a.ms-b.ms}).slice(0,12);var ips=picked.map(function(r){return r.ip});',
    ...
    ' var lines=[];ips.forEach(function(ip){var loc=locationForAddr(ip);OPT.ports.forEach(function(p){if(OPT.protocols.indexOf("vless")>=0)lines.push(vlessLink(ip,"🐱 Cat · "+loc.country+" "+loc.flag+" · VLESS · "+p,OPT.sni,p));if(OPT.protocols.indexOf("trojan")>=0)lines.push(trojanLink(ip,"🐱 Cat · "+loc.country+" "+loc.flag+" · Trojan · "+p,OPT.sni,p));})});',
