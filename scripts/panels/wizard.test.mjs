...
T.__setFetch(async (input, init = {}) => {
  ...
  if (url.endsWith('/health')) {
    ...
    return json({ ok: true, version: isWizard ? T.CAT_WIZARD_VERSION : '5.1.0' });
  }
...
let first;
{
  ...
  check('panel script uploaded (real Cat Panel source)', s && s.script.includes("CAT_PANEL_VERSION = '5.1.0'"));
  ...
  check('online after health probe', first.online === true && first.panelVersion === '5.1.0');
